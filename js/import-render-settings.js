...
document.getElementById('settingsContent').innerHTML=` <div class="settings-group"> <div class="settings-group-title">Version</div> <div class="settings-card"> <div class="settings-row"><div class="settings-row-info"><div class="settings-row-title">Aktueller Stand</div><div class="settings-row-sub">Build ${APP_VERSION}</div></div></div> </div> </div> <div class="settings-group"> <div class="settings-group-title">Erscheinungsbild</div> <div class="settings-card"> <div class="settings-row"> <div class="settings-row-info"> <div class="settings-row-title">Darstellung</div> <div class="settings-row-sub">Hell, Dunkel oder wie das iPhone</div> </div> <div class="segment" id="themeSegment"> <button class="seg-btn ${s.theme==='light'?'active':''}" onclick="setTheme('light')">Hell</button> <button class="seg-btn ${s.theme==='auto'?'active':''}" onclick="setTheme('auto')">Auto</button> <button class="seg-btn ${s.theme==='dark'?'active':''}" onclick="setTheme('dark')">Dunkel</button> </div> </div> </div> </div> <div class="settings-group"> <div class="settings-group-title">Lernen</div> <div class="settings-card"> <div class="settings-row"> <div class="settings-row-info"> <div class="settings-row-title">Neue Karten pro Sitzung</div> <div class="settings-row-sub">Pro Deck, zusätzlich zu fälligen Karten</div> </div> <div class="stepper"> <button class="stepper-btn" onclick="adjustNew(-5)">-</button> <div class="stepper-val" id="newPerDeckVal">${s.newPerDeck}</div> <button class="stepper-btn" onclick="adjustNew(+5)">+</button> </div> </div> <div class="settings-row"><div class="settings-row-info"><div class="settings-row-title">Neue Karten direkt setzen</div><div class="settings-row-sub">1 bis 100</div></div><input class="s-input" id="newPerDeckInput" type="number" min="1" max="100" value="${s.newPerDeck}" onchange="setNewPerDeckFromInput(this.value)" style="max-width:110px"></div> <div class="settings-row"> <div class="settings-row-info"> <div class="settings-row-title">Reihenfolge neue Karten</div> <div class="settings-row-sub">Geordnet = Deck-Reihenfolge (empfohlen)</div> </div> <div class="segment" id="orderSegment"> <button class="seg-btn ${s.newOrder==='ordered'?'active':''}" onclick="setOrder('ordered')">Geordnet</button> <button class="seg-btn ${s.newOrder==='random'?'active':''}" onclick="setOrder('random')">Zufällig</button> </div> </div> </div> </div> <div class="settings-group"><div class="settings-group-title">Deck-Auswahl</div><div class="settings-card" style="padding:16px"><div style="font-size:12px;color:#8888aa;line-height:1.5;margin-bottom:10px">Deaktivierte Decks werden bei „Alle lernen“ und in Fun-Modi ausgeschlossen.</div><button class="s-btn" onclick="showDeckActivationModal()">Decks aktivieren/deaktivieren</button></div></div> <div class="settings-group"><div class="settings-group-title">Fun-Modi (ohne Score-Änderung)</div><div class="settings-card" style="padding:16px"><div style="display:flex;gap:8px;align-items:center;margin-bottom:10px"><label class="s-label" style="margin:0;min-width:95px">Kartenanzahl</label><div class="stepper"><button class="stepper-btn" onclick="adjustFunModeCount(-10)">-</button><div class="stepper-val" id="funModeCountVal">${s.funModeCount||30}</div><button class="stepper-btn" onclick="adjustFunModeCount(+10)">+</button></div></div><div class="settings-row" style="padding:0;border:none;background:transparent"><div class="settings-row-info"><div class="settings-row-title">Kartenanzahl direkt setzen</div><div class="settings-row-sub">5 bis 200</div></div><input class="s-input" id="funModeCountInput" type="number" min="5" max="200" value="${s.funModeCount||30}" onchange="setFunModeCountFromInput(this.value)" style="max-width:110px"></div><div class="segment" id="funDeckFilterSegment" style="margin-bottom:10px"><button class="seg-btn ${s.funModeDeckFilter!=='all'?'active':''}" onclick="setFunDeckFilter('active')">Nur aktive Decks</button><button class="seg-btn ${s.funModeDeckFilter==='all'?'active':''}" onclick="setFunDeckFilter('all')">Alle Decks</button></div><button class="s-btn" onclick="startFunMode('worst')">🎯 Schwächste Karten zufällig</button><button class="s-btn" onclick="startFunMode('random')">🎲 Zufällige gelernte Karten</button><div class="s-hint">Fun-Modus fragt Karten erneut bis alle auf „Leicht“ bewertet wurden. Ranking/Intervalle bleiben unverändert.</div></div></div> <div class="settings-group"> <div class="settings-group-title">☁️ GitHub Sync</div> <div class="settings-card" style="padding:16px"> <div style="font-size:13px;color:#8888aa;margin-bottom:14px;line-height:1.6"> Fortschritt und Deck-Aktivierung werden zwischen iPhone, iPad und Mac synchronisiert. Anfragen brechen nach 10s mit einer Fehlermeldung ab, statt unbegrenzt zu hängen. </div> <div style="display:flex;flex-direction:column;gap:12px"> <div><label class="s-label">GitHub Benutzername</label> <input class="s-input" id="ghUser" placeholder="deinusername" value="${esc(ghConfig.user||'')}"></div> <div><label class="s-label">Repository Name</label> <input class="s-input" id="ghRepo" placeholder="kanji" value="${esc(ghConfig.repo||'')}"></div> <div><label class="s-label">Personal Access Token</label> <input class="s-input" id="ghToken" type="password" placeholder="ghp_xxxxxxxxxxxx" value="${esc(ghConfig.token||'')}"> <div class="s-hint">Settings → Developer settings → Personal access tokens → <b>repo</b></div> </div> </div> <button class="s-btn accent" onclick="saveGHConfig()">Speichern & verbinden</button> ${syncOk?`<button class="s-btn" onclick="ghPull().then(()=>renderHome())">⬇️ Jetzt synchronisieren</button>
<button class="s-btn" onclick="ghPush()">⬆️ Jetzt hochladen</button>`:''} <div id="syncTestResult"></div> </div> </div> <div class="settings-group"> <div class="settings-group-title">💾 Fortschritt</div> <div class="settings-card" style="padding:16px"> <div id="progressSummary" style="margin-bottom:14px"></div> <button class="s-btn" onclick="showInsights()">📈 Lernvorschau öffnen</button> <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px"> <button class="s-btn accent" onclick="exportProgress()">📤 Backup exportieren (.json)</button> <button class="s-btn" onclick="document.getElementById('importProgressInput').click()">📥 Backup importieren</button> <input type="file" id="importProgressInput" accept=".json" style="display:none" onchange="importProgressFile(event)"> <button class="s-btn" onclick="testProgressIntegrity()">🔍 Backup testen</button> <button class="s-btn danger" onclick="confirmReset()">🗑️ Fortschritt zurücksetzen</button> </div> </div> </div> `;
...
const rows = Object.values(decks)
  .sort((a,b)=>a.name.localeCompare(b.name,'de'))
  .map(d=>`<div class="settings-row"><div class="settings-row-info"><div class="settings-row-title">${esc(d.name)}</div><div class="settings-row-sub">${d.cards.length} Karten</div></div><button class="toggle ${isDeckEnabled(d.name)?'on':''}" onclick="toggleDeckEnabled('${esc(d.name)}', this)"></button></div>`)
  .join('');
document.getElementById('settingsContent').innerHTML = `<div class="settings-group"><div class="settings-group-title">Decks aktiv/deaktiv</div><div class="settings-card">${rows || '<div class="settings-row"><div class="settings-row-sub">Keine Decks gefunden.</div></div>'}</div><button class="s-btn accent" onclick="showSettings()" style="margin-top:10px">Zurück</button></div>`;
}

function toggleDeckEnabled(deckName, btn){
const next = !isDeckEnabled(deckName);
userSettings.deckEnabled[deckName] = next;
saveLocal();
